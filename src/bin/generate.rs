...
fn main() {
        ...
        .map(|(d, _)| {
            ...
            chosen
                ...
                .map(|(bitset, choice)| {
                    ...
                    let distribution = cases.into_iter().map(|(case, probability)| {
                        let mut cast: Vec<u8> = choice.iter().map(|i| d[*i]).chain(case).collect();
                        cast.sort();
    ...
    let mut values = vec![0.0f64; 1 << NUMBER_OF_ACTIONS];
    ...
    for finished in (0..1 << NUMBER_OF_ACTIONS).rev().skip(1) {
        ...
        for cast in 0..NUMBER_OF_POSSIBLE_THROWS {
            for cat in 0..NUMBER_OF_ACTIONS {
                if finished & (1 << cat) == 0 {
                    let value = ACTIONS[cat].number(&casts[cast].0) + values[finished | (1 << cat)];
                    if value >= scratch[2][cast] {
                        scratch[2][cast] = value;
                        dp[finished][2][cast] = cat as u8;
                    }
                }
            }
        }
        for i in (0..2).rev() {
            for cast in 0..NUMBER_OF_POSSIBLE_THROWS {
                for (bitset, choice_distribution) in &choice_distributions[cast] {
                    let value = choice_distribution.iter().map(|(j, b)| scratch[i + 1][*j] * b).sum();
                    if value >= scratch[i][cast] {
                        scratch[i][cast] = value;
                        dp[finished][i][cast] = *bitset;
                    }
                }
            }
        }
        values[finished] = scratch[0]
            .iter()
            .zip(&casts)
            .map(|(a, (_, b))| a * b)
            .sum::<f64>();
    ...
    std::fs::File::create(path).unwrap().write_all(slice).unwrap();
}
